...
export default function Reviews() {
  return (
    <section id="avis" className="py-20 px-4 sm:px-6 flex flex-col items-center text-center bg-[#F8F8FC]" aria-label="Avis clients">
      <div className="w-full max-w-3xl">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-6">
          <span className="inline-flex items-center gap-2 text-[11px] font-semibold text-[#CC8F00] uppercase tracking-[.15em] bg-[#FFF8E6] px-4 py-2 rounded-full mb-4">✦ Avis Clients</span>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-[#0D0D1A] tracking-tight mb-2">Ce que disent mes clients</h2>
          <p className="text-sm text-[#666] max-w-md mx-auto">Des retours vérifiés sur la plateforme Comeup.com</p>
        </motion.div>

        {/* Summary bar — dark bg like hero */}
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
          className="relative flex flex-wrap justify-center items-center gap-5 sm:gap-8 rounded-2xl px-6 py-5 mb-6 overflow-hidden border border-[#FFB951]/20"
        ...
        >
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-40 pointer-events-none"
            style={{ background: 'radial-gradient(ellipse,rgba(255,185,81,.12) 0%,transparent 70%)' }} />
          ...
          <div className="w-px h-10 bg-[#FFB951]/20 hidden sm:block" />
          <div className="text-center relative z-10">
            <a href="https://comeup.com/fr/@vianney_hlm" target="_blank" rel="noopener noreferrer"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
          {reviews.map((r, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              whileHover={{ y: -3, borderColor: '#FFE4A0' }}
              className="bg-white border-[1.5px] border-[#EFEFEF] rounded-2xl p-5 flex flex-col gap-3 transition-all duration-300 hover:shadow-lg hover:shadow-[#FFB951]/8"
            >
              <div className="flex gap-1">{[...Array(5)].map((_, j) => <Star key={j} size={11} className="text-[#FFB951] fill-[#FFB951]" />)}</div>
              <p className="text-xs text-[#555] leading-relaxed flex-1">
              ...
              <div className="flex items-center gap-2.5 pt-2.5 border-t border-[#F5F5F5]">
                <div className="w-8 h-8 rounded-lg bg-[#FFF8E6] border border-[#FFE4A0] flex items-center justify-center text-xs font-bold text-[#CC8F00] flex-shrink-0">{r.initials}</div>
                <div>
                  <p className="text-xs font-semibold text-[#0D0D1A]">{r.name}</p>
                  <p className="text-[10px] text-[#AAA]">{r.date}</p>
                </div>
                <span className="ml-auto text-[9px] font-semibold text-[#CC8F00] bg-[#FFF8E6] border border-[#FFE4A0] px-2 py-0.5 rounded-full">Comeup</span>
              </div>
            </motion.div>
          ))}
        </div>
